...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 " style="text-align: center;">Post de {{ post2.username }}</h2>
        </template>
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-4">
                        <h3 class="text-sm font-semibold">Autor:{{ post2.username }}</h3>
                        <p class="text-sm text-gray-500">Publicado el {{ formaterDate(post2.created_at) }}</p>
                        <p class="text-lg font-semibold">{{ post2.title }}</p>
                        <p class="mt-2">{{ post2.content }}</p>
                        <div>
                                    <button type="button" @click="editpost(post2.id,)" v-if="post.user_id==$page.props.auth.user.id"  class="text-green-700 hover:text-white border border-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-green-500 dark:text-green-500 dark:hover:text-white dark:hover:bg-green-600 dark:focus:ring-green-800">Editar</button>
                                    <button type="button" @click="eliminarpost(post2.id)" v-if="post.user_id==$page.props.auth.user.id || $page.props.auth.user.role_id==1 && post2.is_active == true" class="text-red-700 hover:text-white border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">Eliminar</button>
                        </div>
                    </div>
                </div>
                <div class="mt-8"></div>
                    ...
                    <div class="p-4 ">
                        ...
                        <ul style="  overflow-y: auto;   max-height: 400px; " >
                            <li v-for="comment in comments2" :key="comment.id">
                                ...
                                <h6 v-if="comment.updated_at" class=" items-center text-xs font-normal text-gray-500 hover:underline dark:text-gray-400">
                                ...
                                <div>
                                    ...
                                    <button type="button" @click="eliminarcomment(comment.id)" v-if="comment.user_id==$page.props.auth.user.id || $page.props.auth.user.role_id==1" class="text-red-700 hover:text-white border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">Eliminar</button>
                                </div>
                 </li>
                        </ul>
    ...
    methods: {
        ...
        pollRoute() {
            this.pollInterval = setInterval(() => {
                axios.get(route('post.show.json', { id: this.post.id }))
                    .then(response => {
                        console.log(response.data);
                        this.post2 = response.data.post;
